...
import fr.upem.algoproject.DijkstraWalker;
import fr.upem.algoproject.Maze;
import fr.upem.algoproject.MazeExport;
import fr.upem.algoproject.MazeLoader;
import fr.upem.algoproject.Point;
...
public class ProjetAlgo {
	public static void main(String[] args) {
		...
		FlaggedOption input = new FlaggedOption("inputXMLFile")
				.setStringParser(JSAP.STRING_PARSER).setShortFlag('i')
				.setRequired(true).setLongFlag(JSAP.NO_LONGFLAG);

		FlaggedOption output = new FlaggedOption("outputImage")
				.setStringParser(JSAP.STRING_PARSER).setShortFlag('o')
				.setRequired(true).setLongFlag(JSAP.NO_LONGFLAG);
		Switch shortestPath = new Switch("withShortestPath").setShortFlag('p')
				.setLongFlag("with-shortest-path");
		try {
		...
		if (!config.success()) {
			...
			System.out.println("java " + ProjetAlgo.class.getCanonicalName()
					+ " " + jsap.getUsage());
			System.exit(1);
		}
		...
		try {
			...
			Maze m = MazeLoader.loadMazeFromFile(Paths.get(config
					.getString("inputXMLFile")));
			long loadEnd = System.currentTimeMillis();
			l.info("Load phase took " + (loadEnd - loadStart)
					+ "ms to complete.");
			...
			if (config.getBoolean("withShortestPath")) {
				...
				l.info("Phase Dijkstra took " + (dijkstraEnd - dijkstraStart)
						+ "ms to complete.");
			}

			MazeExport.saveToPng(m, Paths.get(config.getString("outputImage")),
					config.getBoolean("withShortestPath"));
